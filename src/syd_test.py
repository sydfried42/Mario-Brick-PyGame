...
pygame.display.set_caption('Mario Brick Breaker')
        ...
        for row in range(rows):
            ...
            for col in range(cols):
                ...
                if row < 2:
                    ...
                    strength = 1
                block_individual = [rect, strength]
                while self.check_collision(block_individual):
                    block_x = random.randint(0, screen_width - self.width) 
                    block_y = random.randint(0, screen_height - self.height)
                    rect = pygame.Rect(block_x, block_y, self.width, self.height)
                    block_individual[0] = rect
                block_row.append(block_individual)
            self.blocks.append(block_row)
    def check_collision(self, block):
        for row in self.blocks:
            for existing_block in row:
                if existing_block[0] != block[0] and existing_block[0].colliderect(block[0]):
                    return True
        return False
# Paddle class
class Paddle():
...
while run:
    ...
    if not live_ball:
        if game_over == 0:
            ...
            draw_text('Press C to create chaos!!', font, text_col, 100, screen_height // 2 + 100)
        elif game_over == 1:
